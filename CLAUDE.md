...
- Keep dependencies at zero (stdlib `urllib` for HTTP, `osascript`/`ioreg` for system queries).
- Run all scripts from `~/.apptracker/` working directory (not the repo).
...
- IF changing launchd plists → THEN update the hardcoded paths (`/Users/hsp/.apptracker/`) if the username changes.
